...
def _client_request_id(request: Request) -> str | None:
    return request.headers.get("x-ms-client-request-id")
...
async def enforce_runtime_boundary(request: Request, call_next: Any) -> Response:
    ...
    _record_metrics(response.status_code, perf_counter() - started)
...
def main() -> int:
    ...
    host = "127.0.0.1" if settings.environment == "local" else "0.0.0.0"  # nosec B104
    uvicorn.run("bive.api:app", host=host, port=8080, reload=False)  # nosec B104
    return 0
